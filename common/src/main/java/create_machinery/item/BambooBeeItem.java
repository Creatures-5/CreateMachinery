...
public class BambooBeeItem extends MachineryItem {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level world, List<Component> tooltip, TooltipFlag context) {
        ...
        if (positions.isEmpty()) {
            tooltip.add(Component.translatable("gui.create_machinery.bamboo_bee.tooltip.help1"));
            tooltip.add(Component.translatable("gui.create_machinery.bamboo_bee.tooltip.help2"));
        } else {
            tooltip.add(Component.translatable("gui.create_machinery.bamboo_bee.tooltip.header", positions.size()));

            for (int i = 0; i < positions.size(); i++) {
                CompoundTag tag = positions.getCompound(i);
                String name = tag.getString("name");
                boolean input = tag.getBoolean("input");
                String key = "gui.create_machinery.bamboo_bee.tooltip." + (input ? "input" : "output");
                tooltip.add(
    ...
    private void send(String message, Player player, ChatFormatting formatting) {
        if (player != null) {
            player.sendSystemMessage(Component.translatable("gui.create_machinery.bamboo_bee." + message).withStyle(formatting));
        }
    }
